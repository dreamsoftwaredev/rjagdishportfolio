...
const skillsData = [
    ...
    { name: "Axios", icon: <SiAxios className="text-purple-600 text-4xl" /> },
...
const Skills = () => {
    return (
        <section className='min-h-screen bg-black p-10'>
            <h1 className='text-4xl text-center text-white font-bold mb-10'>My Skills</h1>
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 justify-items-center'>
                {skillsData.map((skill, index) => (
                    <div key={index} className="flex flex-col items-center bg-gray-800 p-6 rounded-xl shadow-lg w-44 transform hover:scale-110 transition-transform duration-300">
                        {skill.icon}
                        <p className='text-white mt-2 font-medium'>{skill.name}</p>
                    </div>
                ))}
            </div>
        </section>
